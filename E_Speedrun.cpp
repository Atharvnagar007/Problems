...
int query(int v, int tl, int tr, int l, int r) {
    if (l > r) return 0;
    propagate(v, tl, tr); // Apply lazy updates before querying
...
int main() {
    ...
    int num_queries;
    cin >> num_queries; // Renamed to avoid conflict with the loop variable

    for (int i = 1; i <= num_queries; i++) { // Start from 1
        int v, x;
        cin >> v >> x;
